...
const BTCPriceWidget = () => {
    ...
    return (
        <div className="card bg-white text-gray-900 overflow-hidden relative min-h-[160px] flex flex-col justify-between border border-gray-100 shadow-sm">
            <div className="absolute top-0 right-0 -mt-2 -mr-2 p-4 opacity-5">
            ...
            <div className="relative z-10 p-1">
                <div className="flex justify-between items-start">
                    <div>
                        <p className="text-gray-500 text-sm font-medium">Bitcoin Price (INR)</p>
                        <h2 className="text-3xl font-bold mt-2 text-gray-900">{formatINR(btcPrice)}</h2>
                    </div>
                    <div className={`flex items-center px-2 py-1 rounded-full ${isPositive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        ...
                        <span className="text-sm font-bold">{Math.abs(change24h).toFixed(2)}%</span>
                    </div>
                </div>

                <div className="mt-8 flex items-center justify-between">
                    <div className="text-xs text-gray-500 flex items-center">
                        <div className={`h-2 w-2 rounded-full mr-2 ${isRefreshing ? 'bg-bitcoin-orange animate-pulse' : 'bg-green-500'}`}></div>
                        Live Updates
                    </div>
                    <button
                        onClick={fetchPrice}
                        className={`text-gray-400 hover:text-gray-600 transition-all p-2 rounded-full hover:bg-gray-100 ${isRefreshing ? 'animate-spin' : ''}`}
                        title="Refresh Price"
                    >
